...
public abstract class AACPlayer {
     ...
     * 
     * @see setAudioBufferCapacityMs(int) Default = 1500
     */
    ...
    /*
     * protected void dumpHeaders( URLConnection cn ) { for
     * (java.util.Map.Entry<String, java.util.List<String>> me :
     * cn.getHeaderFields().entrySet()) { for (String s : me.getValue()) {
	...
	return computeInputBufferSize(info.getRoundBytesConsumed(), info.getRoundSamples(), info.getSampleRate(), info.getChannels(), durationMs);
    }

    protected static int computeInputBufferSize(int bytesconsumed, int samples, int sampleRate, int channels, int durationMs) {
