...
public abstract class PCMFeed implements Runnable, AudioTrack.OnPlaybackPositionUpdateListener {
     ...
     * @param channels
     *            the number of channels - only allowed values are 1 (mono) and
     *            2 (stereo).
    ...
    /**
     * Stops the PCM feeder. This method just asynchronously notifies the
     * execution thread. This can be called in any state.
     */
     ...
     * 
     * @param samples
     *            the size of the buffer in samples (all channels)
     * @return the time in milliseconds
     */
    ...
    /**
     * Called on the listener to notify it that the previously set marker has
     * been reached by the playback head.
     */
    ...
    public void run() {
	Log.d(LOG, "PCMFEED run(): sampleRate=" + sampleRate + ", channels=" + channels + ", bufferSizeInBytes=" + bufferSizeInBytes + " (" + bufferSizeInMs
		+ " ms)");

	AudioTrack atrack = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate, channels == 1 ? AudioFormat.CHANNEL_CONFIGURATION_MONO
		: AudioFormat.CHANNEL_CONFIGURATION_STEREO, AudioFormat.ENCODING_PCM_16BIT, bufferSizeInBytes, AudioTrack.MODE_STREAM);
	...
	while (!stopped) {
		...
		// Log.d( LOG, "PCM fed by " + ln + " and written " + written +
		// " samples - buffered " + buffered);
		// Log.d( LOG, "current buffered " + buffered +
		// " SET-bufferSizeInBytes " + bufferSizeInBytes + " ms " +
		// bufferSizeInMs);
		// Karthik below in the if clause 44100 was previously
		// bufferSizeInBytes
		if (!isPlaying) {

		    if (buffered >= 44100) {
			// Log.d( LOG, "start of AudioTrack - buffered " +
			// buffered + " samples");
			Log.d(LOG, "current buffered " + buffered + " SET-bufferSizeInBytes " + bufferSizeInBytes + " LOCAL SEC or ms "
				+ (bufferSizeInBytes / (sampleRate * 2)));
			atrack.play();
			isPlaying = true;
		    } else {
    ...
    /**
     * Releases the lsamples variable. This method is called always after
     * processing the acquired lsamples.
     */
