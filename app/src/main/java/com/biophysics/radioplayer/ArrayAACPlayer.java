...
/**
 * This is the AACPlayer which uses ArrayDecoder to decode AAC stream into PCM
 * samples.
 * 
 * <pre>
...
public class ArrayAACPlayer extends AACPlayer {
     ...
     */
    public ArrayAACPlayer(ArrayDecoder decoder, PlayerCallback playerCallback, int audioBufferCapacityMs, int decodeBufferCapacityMs) {
     ...
     *            the expected average bitrate in kbit/sec This calls the JNI
     *            code
     */
	...
	try {
		...
		Log.i(LOG, "play(): average rate (samples/sec): audio=" + profSampleRate + ", decoding=" + (1000 * profSamples / profMs) + ", audio/decoding= "
			+ perf + " %  (the higher, the better; negative means that decoding is slower than needed by audio)");
	    }
